...
*   **SkyLogg.Server.Api**: Houses API controllers, mappers, the `DbContext`, EF Core migrations, email templates, action filters, SignalR hubs, and server-specific configuration.
*   **SkyLogg.Server.Web**: The application's default startup project and entry point. It hosts `App.razor` and configures Blazor Server and server-side rendering (SSR).
*   **SkyLogg.Server.Shared**: (Also known as Aspire's ServiceDefaults) Contains common code shared between the `SkyLogg.Server.Api` and `SkyLogg.Server.Web` projects.
*   **SkyLogg.Server.AppHost**: Manages the .NET Aspire configuration and orchestration.
*   **SkyLogg.Shared**: Contains shared DTOs, enums, custom exceptions, shared services, and `.resx` resource files.
*   **SkyLogg.Tests**: Contains all UI and integration tests.
*   **SkyLogg.Client.Core**: The heart of the client application. Contains all shared Blazor components, pages, layouts, client-side services, and the primary `App.ts` and `App.scss` files.
*   **SkyLogg.Client.Web**: The Blazor WebAssembly (WASM) standalone project.
*   **SkyLogg.Client.Maui**: The .NET MAUI Blazor Hybrid project for native mobile and desktop apps.
*   **SkyLogg.Client.Windows**: The Windows Forms Blazor Hybrid project.
...
-   **Build the project**: Run `dotnet build` in SkyLogg.Server.Web project root directory.
-   **Run the project**: Run `dotnet run` in SkyLogg.Server.Web project root directory.
-   **Run tests**: Run `dotnet test` in SkyLogg.Tests project root directory.
-   **Add new migrations**: Run `dotnet ef migrations add <MigrationName> --output-dir Data/Migrations --verbose` in SkyLogg.Server.Api project root directory.
-   **Generate Resx C# code**: Run `dotnet build -t:PrepareResources` in SkyLogg.Shared project root directory.
