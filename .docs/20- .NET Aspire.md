...
Welcome to Stage 20! In this stage, you'll learn about .NET Aspire and how it transforms both development and deployment experiences in this SkyLogg project.
...
**Location**: [`src/Server/SkyLogg.Server.AppHost/`](/src/Server/SkyLogg.Server.AppHost/)
...
1. Open `Program.cs` in the `SkyLogg.Server.AppHost` project
...
**Location**: [`src/Server/SkyLogg.Server.AppHost/appsettings.Development.json`](/src/Server/SkyLogg.Server.AppHost/appsettings.Development.json)
...
- ❌ `SkyLogg.Server.Api` **does NOT** run on Docker
- ❌ `SkyLogg.Server.Web` **does NOT** run on Docker
...
3. Run this command in the terminal to clean previous Windows build artifacts:
   ...
   ```
4. Then run the `SkyLogg.Server.AppHost` project
