...
4. [SkyLogg.Client.Windows Advantages](#skyloggclientwindows-advantages) - Better Windows support
...
The `SkyLogg.Client.Maui` and `SkyLogg.Client.Windows` projects demonstrate this architecture in action.
...
**Location**: [`/src/Client/SkyLogg.Client.Maui/Platforms/Android/MainActivity.cs`](/src/Client/SkyLogg.Client.Maui/Platforms/Android/MainActivity.cs)
...
#### iOS - Universal Links
For iOS, this is handled automatically by the `apple-app-site-association` file in the `wwwroot` of the `SkyLogg.Client.Web` project. No manual configuration is needed per page!

**Location**: [`/src/Client/SkyLogg.Client.Web/wwwroot/.well-known/apple-app-site-association`](/src/Client/SkyLogg.Client.Web/wwwroot/.well-known/apple-app-site-association)
...
### ApplicationVersion in SkyLogg.Client.Maui

The `ApplicationVersion` property in the `SkyLogg.Client.Maui.csproj` file manages the app's version for store submissions and updates.
...
## SkyLogg.Client.Windows Advantages

The `SkyLogg.Client.Windows` project offers several advantages over the MAUI Windows output:

### Key Benefits

1. **Broader OS Support**: Works on Windows 7, 8, 10, and 11
   - In contrast, `SkyLogg.Client.Maui`'s Windows output only works on Windows 10 and 11
...
The app checks the WebView version on Android to ensure compatibility in [`App.xaml.cs`](/src/Client/SkyLogg.Client.Maui/App.xaml.cs):
...
The MAUI project is located at: `src/Client/SkyLogg.Client.Maui/`
...
The [`MauiProgram.cs`](/src/Client/SkyLogg.Client.Maui/MauiProgram.cs) file is the starting point of the MAUI application:
...
Services specific to MAUI are registered in [`MauiProgram.Services.cs`](/src/Client/SkyLogg.Client.Maui/MauiProgram.Services.cs):
...
The [`MainPage.xaml`](/src/Client/SkyLogg.Client.Maui/MainPage.xaml) file hosts the `BlazorWebView` component, which renders Blazor content inside a native WebView:
...
- It renders the `Routes` component (from `SkyLogg.Client.Core`) inside the `#app-container` element
- All your Blazor components render inside this WebView, just like in the browser!

### 4. BlazorWebView Customization

The `SetupBlazorWebView()` method in [`MauiProgram.cs`](/src/Client/SkyLogg.Client.Maui/MauiProgram.cs) customizes the WebView for each platform:
...
Implements `IStorageService` using MAUI's `Preferences` API in [`MauiStorageService.cs`](/src/Client/SkyLogg.Client.Maui/Infrastructure/Services/MauiStorageService.cs):
...
Implements `IExternalNavigationService` for opening external URLs in [`MauiExternalNavigationService.cs`](/src/Client/SkyLogg.Client.Maui/Infrastructure/Services/MauiExternalNavigationService.cs):
