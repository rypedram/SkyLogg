...
`AppOfflineDbContext` migrations are slightly different from SkyLogg.Server.Api's `AppDbContext` migrations.
To add migration for `AppOfflineDbContext` first set `SkyLogg.Server.Web` as the Startup Project in solution explorer and set `SkyLogg.Client.Core` it as the Default Project in Package Manager Console and run the following commands:
```powershell
Add-Migration Initial -OutputDir Infrastructure\Data\Migrations -Context AppOfflineDbContext -Verbose
```
Or open a terminal in your SkyLogg.Server.Web project directory and run followings:
```bash
dotnet tool restore && dotnet ef migrations add Initial --context AppOfflineDbContext --output-dir Infrastructure/Data/Migrations --project ../Client/SkyLogg.Client.Core/SkyLogg.Client.Core.csproj --verbose
```
...
1. Make sure `Server.Web` is set as the default startup project, and `SkyLogg.Client.Core` is the default project in the Package Manager Console.

2. Run the following command:

```powershell
Optimize-DbContext -Context AppOfflineDbContext -OutputDir Infrastructure/Data/CompiledModel -Namespace SkyLogg.Client.Core.Infrastructure.Data -Verbose
```

**OR** Run the following command in SkyLogg.Server.Web directory:
```bash
dotnet tool restore && dotnet ef dbcontext optimize --context AppOfflineDbContext --output-dir Infrastructure/Data/CompiledModel --namespace SkyLogg.Client.Core.Infrastructure.Data --project ../../Client/SkyLogg.Client.Core/SkyLogg.Client.Core.csproj --verbose
```
